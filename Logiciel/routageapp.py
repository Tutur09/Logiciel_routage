...
class RoutageApp(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        # 🔹 Supprimer le fond rouge de widgetRoutage
        self.widgetRoutage.setStyleSheet("")  # Réinitialiser le fond

        # 🔹 Ajouter la carte Matplotlib dans widgetRoutage
        self.figure = Figure(figsize=(10, 8))
        self.canvas = FigureCanvas(self.figure)

        # 🔹 Assurer que widgetRoutage utilise un layout sans marges
        layout = QVBoxLayout(self.widgetRoutage)
        layout.setContentsMargins(0, 0, 0, 0)  # Supprime toutes les marges
        layout.setSpacing(0)  # Évite tout espace entre les widgets
    ...
    def init_map(self):
        """Initialise la carte Cartopy sans marges pour occuper tout l'espace"""
        self.ax = self.figure.add_subplot(111, projection=ccrs.PlateCarree())
        self.ax.set_extent([-180, 180, -90, 90], crs=ccrs.PlateCarree())

        # 🔹 Forcer Matplotlib à occuper tout l'espace
        self.figure.subplots_adjust(left=0, right=1, top=1, bottom=0)
        self.ax.set_position([0, 0, 1, 1])

        # 🔹 Ajuster la taille dynamique de la figure
        self.canvas.resize(self.widgetRoutage.width(), self.widgetRoutage.height())
        self.figure.set_size_inches(self.widgetRoutage.width() / self.figure.dpi, 
                                    self.widgetRoutage.height() / self.figure.dpi, forward=True)

        # 🔹 Supprimer les axes et ticks pour éviter les marges
        self.ax.set_xticks([])
        self.ax.set_yticks([])
